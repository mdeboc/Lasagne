...
def nadam(loss_or_grads, params, learning_rate=0.001, beta1=0.9,
    ...
    Nadam updates implemented as in [1]_.
    ...
    Notes
    -----
    The paper [1]_ improves the Adam optimization algorithm with
    an accelerated gradient a la Nesterov.

    References
    ----------
    .. [1] Timothy Dozat (2015):
    ...
    for param, g_t in zip(params, all_grads):
        ...
        m_t = beta1*m_prev + (one-beta1)*g_t
        v_t = beta2*v_prev + (one-beta2)*g_t**2
        step = a_t*(beta1*m_t + (one-beta1)*g_t/(one-beta1**t)) /\
               (T.sqrt(v_t) + epsilon)
